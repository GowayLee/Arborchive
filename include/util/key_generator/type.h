...
// For clang::QualType
static inline KeyType makeKey(const QualType &qualType, const ASTContext &ctx) {
  std::string s;
...
// For clang::NamedDecl
static inline KeyType makeKey(const NamedDecl *decl, const ASTContext &ctx) {
  // 1. 作用域路径
...
static inline KeyType makeKey(const TemplateDecl *decl, const ASTContext &ctx) {
...
static inline KeyType makeKey(const TypeDecl *decl, const ASTContext &ctx) {
  // return makeKey(cast<NamedDecl>(decl), ctx);
  if (auto *namedDecl = dyn_cast<NamedDecl>(decl))
    return makeKey(namedDecl, ctx);
  else {
