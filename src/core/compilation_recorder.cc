...
int CompilationRecorder::createCompilation(
  ...
  auto compilation_model =
      std::make_shared<CompilationModel>(working_directory);
...
void CompilationRecorder::recordArguments(
  ...
  for (const auto &arg : flags) {
    auto args_model =
        std::make_shared<CompilationArgsModel>(compilation_id_, arg_num++);
...
void CompilationRecorder::recordTime(CompilationTimeKind kind, double seconds) {
  auto model = std::make_shared<CompilationTimeModel>(compilation_id_,
                                                      time_record_seq_++);
  model->setKind(kind);
...
void CompilationRecorder::recordFile(const std::string &file) {
  auto fileModel = std::make_shared<FileModel>(file);
  auto containerModel = std::make_shared<ContainerModel>(
...
void CompilationRecorder::finalize(double total_cpu, double total_elapsed) {
  auto finished_model =
      std::make_shared<CompilationFinishedModel>(compilation_id_);
